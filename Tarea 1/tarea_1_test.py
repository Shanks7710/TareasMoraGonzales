...
def test_casos_error_filtrar_vocales():
    ...
    estado, res = Tarea_1_Gabriel_y_Hillary.filtrar_vocales(cadena=123, bandera=True)
    assert estado == -100
    assert res is None
...
def test_casos_error_encontrar_extremos():
    ...
    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos("no es lista")
    assert estado == -600
    assert minimo is None
    assert maximo is None

    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos(123)
    assert estado == -600
    assert minimo is None
    assert maximo is None

    # Error si contiene elementos no numéricos
    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos([1, 2, "3", 4])
    assert estado == -700
    assert minimo is None
    assert maximo is None

    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos([1, 2, True, 4])
    assert estado == -700
    assert minimo is None
    assert maximo is None

    # Error si la lista está vacía
    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos([])
    assert estado == -800
    assert minimo is None
    assert maximo is None
    ...
    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos(lista_larga)
    assert estado == -900
    assert minimo is None
    assert maximo is None
...
def test_casos_exito_encontrar_extremos():
    ...
    estado, minimo, maximo = Tarea_1_Gabriel_y_Hillary.encontrar_extremos(lista_prueba)
    assert estado == 0
    assert minimo == 1
    assert maximo == 9
