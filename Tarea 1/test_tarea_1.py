...
def test_casos_error_filtrar_vocales():
    ...
    estado, res = (
        tarea_1_example_solution.filtrar_vocales(cadena=123, bandera=True)
    )
    assert estado == -100
    assert res is None
...
def test_casos_error_encontrar_extremos():
    ...
    estado, minimo, maximo = (
        tarea_1_example_solution.encontrar_extremos("no es lista")
    )
    assert estado == -600
    ...
    estado, minimo, maximo = (
        tarea_1_example_solution.encontrar_extremos([1, 2, "3", 4])
    )
    assert estado == -700
    assert minimo is None
    assert maximo is None
    ...
    estado, minimo, maximo = (
        tarea_1_example_solution.encontrar_extremos([1, 2, True, 4])
        )
    assert estado == -700
    assert minimo is None
    assert maximo is None
    ...
    estado, minimo, maximo = (
        tarea_1_example_solution.encontrar_extremos(lista_larga)
    )
    assert estado == -900
    assert minimo is None
    assert maximo is None
...
def test_casos_exito_encontrar_extremos():
    ...
    estado, minimo, maximo = (
        tarea_1_example_solution.encontrar_extremos(lista_prueba)
        )
    assert estado == 0
    assert minimo == 1
    assert maximo == 9
    ...
    # Caso aleatorio
    lista_prueba = (
        [random.randint(-100, 100) for _ in range(random.randint(2, 15))]
    )
